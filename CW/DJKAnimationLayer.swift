...
class DJKAnimationLayer: CATransformLayer {
   ...
   lazy var backLayer:CALayer = {
    ...
    func setTheFrontLayer(image:UIImage) {
        ...
        let rightImgRef = CGImageCreateWithImageInRect(tmpImageRef, CGRectMake(image.size.width/2 * UIScreen.mainScreen().scale, 0, image.size.width/2 * UIScreen.mainScreen().scale, image.size.height * UIScreen.mainScreen().scale))
    ...
    func setTheBackLayer(image:UIImage) {
        let tmpImageRef = image.CGImage
        let rightImgRef = CGImageCreateWithImageInRect(tmpImageRef, CGRectMake(0, 0, image.size.width/2 * UIScreen.mainScreen().scale, image.size.height * UIScreen.mainScreen().scale))
